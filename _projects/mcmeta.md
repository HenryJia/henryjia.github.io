...
However, Adam is old, really old. The original ArXiv link dates back to 2014. That's 12 years ago at the time of writing (2026). To put things in perspective, when Adam was first released, Theano was the only deep learning framework in Python. TensorFlow didn't exist. Even Keras wasn't released yet. Plenty of people were still writing frameworks in C++ even. Since then the deep learning field has been a battle royale of frameworks, models, and methodologies. We rarely see anything survive for even a few years without getting completely dominated by something else. Yet, Adam somehow has survived all of this.
...
**In effect, we are using the minor component subspace as our collective variable for metadynamics. We are trying to find the escape dimensions without having to pay the cost of massive overparameterisation.**

The second big issue, the one which I've yet to figure out how to solve fully, is the hyperparameters. Metadynamics introduces a whole new set of hyperparameters. We have the height and width of the Gaussian functions, and how often we add them. These hyperparameters are very important for the performance of metadynamics. If we add the Gaussians too often, or if they're too high, we might end up adding too much energy to the system, which can lead to divergence. If we don't add them often enough, or if they're too low, we might not add enough energy to actually be useful.
